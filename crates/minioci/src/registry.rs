...
use bytes::Bytes;
use oci_spec::image::{Descriptor, ImageIndex, ImageManifest, MediaType, ToDockerV2S2};
use reqwest::{Client, RequestBuilder, Response, StatusCode};
use tokio::{fs::File, io::AsyncWriteExt};
use url::Url;

use crate::{data::OciStruct, name::ImageName, platform::OciPlatform, progress::OciBoundProgress};

#[derive(Clone)]
pub struct OciRegistryResolution {
    pub manifest: OciStruct<ImageManifest>,
...
impl OciRegistryClient {
    pub fn new(client: Client, url: Url, token: Option<String>) -> Result<OciRegistryClient> {
    ...
    async fn get_raw_manifest_with_digest<N: AsRef<str>, R: AsRef<str>>(
        ...
        name: N,
        reference: R,
    ) -> Result<(OciStruct<ImageManifest>, String)> {
        let url = self.url.join(&format!(
            "/v2/{}/manifests/{}",
            name.as_ref(),
            reference.as_ref(),
        ...
        Ok((OciStruct::new(bytes.to_vec(), manifest), digest))
    }

    pub async fn resolve<N: AsRef<str>, R: AsRef<str>>(
        &mut self,
        name: N,
        reference: Option<R>,
        digest: Option<N>,
        platform: &OciPlatform,
    ) -> Result<OciRegistryResolution> {
        let what = digest
            .as_ref()
        ...
        let bytes = response.bytes().await?;
        let manifest = serde_json::from_slice(&bytes)?;
        Ok(OciRegistryResolution {
            manifest: OciStruct::new(bytes.to_vec(), manifest),
            descriptor: None,
            digest,
        })
    }

    fn pick_manifest(&mut self, platform: &OciPlatform, index: ImageIndex) -> Option<Descriptor> {
        for item in index.manifests() {
            if let Some(offer) = item.platform() {
                if *offer.os() == platform.os
                    && *offer.architecture() == platform.arch
                {
