...
impl fmt::Display for ImageName {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if ImageName::DOCKER_HUB == self.hostname && self.port.is_none() {
...
impl ImageName {
    pub const DOCKER_HUB: &'static str = "registry.docker.io";
    ...
    pub fn parse(name: &str) -> Result<Self> {
        ...
        let (mut hostname, mut name) = name
            ...
            .unwrap_or_else(|| {
                (
                    ImageName::DOCKER_HUB.to_string(),
                    format!("library/{}", name),
                )
            });
        ...
        if !hostname.contains('.') && full_name.chars().filter(|x| *x == '/').count() == 1 {
            ...
            hostname = ImageName::DOCKER_HUB.to_string();
        }
    ...
    pub fn normalize(&self) -> ImageName {
        ImageName {
            hostname: self.hostname.clone(),
            port: self.port.clone(),
            name: self.name.clone(),
            reference: if self.digest.is_some() {
                None
            } else {
