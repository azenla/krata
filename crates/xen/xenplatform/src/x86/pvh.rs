...
use crate::{
    ...
    x86::acpi::{
        acpi_build_tables, acpi_config, acpi_ctxt, acpi_mem_ops, dsdt_pvh, hvm_info_table,
    },
};
...
impl BootSetupPlatform for X86PvhPlatform {
    ...
    async fn alloc_magic_pages(&mut self, domain: &mut BootDomain) -> Result<()> {
        ...
        domain.console_mfn = special_pfn(SPECIALPAGE_CONSOLE);
        domain
            .phys
    ...
    async fn bootlate(&mut self, domain: &mut BootDomain) -> Result<()> {
        ...
        domain
            .call
            .set_hvm_param(domain.domid, HVM_PARAM_CONSOLE_EVTCHN, domain.console_mfn)
            .await?;
        Ok(())
    }
    ...
    async fn gnttab_seed(&mut self, domain: &mut BootDomain) -> Result<()> {
        let console_gfn = domain.console_mfn;
        let addr = domain
            .call
